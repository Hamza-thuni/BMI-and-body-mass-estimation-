...
----------------------
V8: Hybrid Live Demo — Physics + Deep Features + Pose.
...
def predict_weight(bundle, features):
    ...
    # Apply PCA if model has it (hybrid model)
    if "pca" in bundle and bundle["pca"] is not None:
        Xs = bundle["scaler"].transform(X)
        Xp = bundle["pca"].transform(Xs)
    elif "poly" in bundle and bundle["poly"] is not None:
...
def main():
    print("\n" + "=" * 50)
    print(" V8 Hybrid Live Demo (Physics + Deep + Pose)")
    print("=" * 50)
    ...
    while True:
        ...
        if res and res.pose_landmarks:
            ...
            if weight_hist:
                ...
                label = "V8 Hybrid" if is_hybrid else "V8 Physics"
        ...
        cv2.imshow("V8 Weight & BMI Estimation", display)
