...
def extract_for_split(split_name: str,
                      img_root: str,
                      batch_size: int,
    ...
    for imgs, meta, bmi, fname, path in tqdm(dl, desc=f"{split_name}"):
        imgs = imgs.to(device)
        with torch.no_grad():
            deep = model(imgs)   # (B, C)

        # anthropometric per image using OpenCV + mediapipe
        for p in path:
            bgr = cv2.imread(p)
            feats = extract_mixed_features(bgr)
...
if __name__ == "__main__":
    ...
    out_dir = str(features_dir("v1"))
    os.makedirs(out_dir, exist_ok=True)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Device:", device)

    feat_model = ResNetFeatureExtractor("resnet50", pretrained=True).to(device)
    ...
    for split, img_root in splits.items():
        ...
        np.savez_compressed(
            os.path.join(out_dir, f"{split}_features.npz"),
            deep=deep,
            antro=antro,
            bmi=bmi,
            names=names,
